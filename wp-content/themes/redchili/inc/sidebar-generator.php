...
if ( !class_exists( 'RDTheme_Sidebar_Generator' ) ) {
	...
	class RDTheme_Sidebar_Generator {
			...
			<div class="widgets-holder-wrap">
				<div id="rdtheme-new-sidebar" class="widgets-sortables">
					<div class="sidebar-name">
						...
						<h2><?php esc_html_e( 'Add New Sidebar', 'redchili' ); ?><span class="spinner"></span></h2>
					</div>
					<div class="sidebar-description">
						<form style="padding:0 7px;" method="POST" action="<?php echo esc_url( admin_url( 'admin-ajax.php?action=rdtheme_add_sidebar' ) );?>">
							...
							<table class="form-table">
								<tr>
									<th scope="row"><?php esc_html_e( 'Name', 'redchili' ) ?></th>
									<td><input type="text" class="text" name="name" value=""></td>
									<td><input type="submit" class="button-primary" value="<?php esc_html_e( 'Add', 'redchili' ) ?>"></td>
								</tr>
							</table>
						</form>
					</div>
		...
		public function ajax_add_sidebar() {
			...
			if ( empty( $name ) ) {
				wp_send_json_error( esc_html__( "Sidebar name can't be empty", 'redchili' ) );
			}
			if ( empty( $nonce ) ) {
				wp_send_json_error( esc_html__( 'Empty nonce', 'redchili' ) );
			}
			if ( ! wp_verify_nonce( $nonce, 'rdtheme_add_sidebar' ) ) {
				wp_send_json_error( esc_html__( 'Invalid nonce', 'redchili' ) );
			}
			...
			if ( array_key_exists( $id, $sidebars ) ) {
				wp_send_json_error( esc_html__( 'Sidebar with the same name already exists. Please choose a different name', 'redchili' ) );
			}
		...
		public function ajax_remove_sidebar() {
			...
			if ( empty( $id ) ) {
				wp_send_json_error( esc_html__( 'Sidebar ID not found', 'redchili' ) );
			}
			if ( empty( $nonce ) ) {
				wp_send_json_error( esc_html__( 'Empty nonce', 'redchili' ) );
			}
			...
			<script type="text/javascript">
				(function($){
							...
							complete: function(response) {
								...
								if ( ! response || ! response.responseJSON || ! response.responseJSON.success) {
									if (response && response.responseJSON && response.responseJSON.data) {
										...
										alert('<?php esc_html_e( 'Operation failed', 'redchili' ); ?>');
									}
								}
								else {
					...
					$(document).on('click','#widgets-right .sidebar-rdtheme-custom .sidebar-name h2 .sidebar-rdtheme-custom-closebtn',function(event){
						...
						if (confirm('<?php esc_html_e( 'Are you sure you want to remove this custom sidebar', 'redchili' ); ?>')) {
