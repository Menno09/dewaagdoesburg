...
if ($source == 'product' && $wc == true) {
    ...
    if ($_product->is_purchasable()) {
        if ($_product->is_in_stock()) {
            $quantity_html = sprintf('<div class="fmp-wc-quantity-wrap"><input type="number" name="quantity" step="%1$d" min="%1$d" title="%2$s" size="4" pattern="[0-9]*" inputmode="numeric" value="%1$d"></div>', 1, __("Quantity", "food-menu-pro"));
